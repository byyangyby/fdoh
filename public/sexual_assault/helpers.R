...
# Adjust color
####
tr <- function(color, alpha = 0.6){
  x <- adjustcolor(color, alpha.f = alpha)
  return(x)
}
...
choro <- function(
  ...
  main = NULL,
  var = NULL,
  cols = c("lightblue", "white", "darkorange"),
  legend_round = 2,
  ...
  add = FALSE, 
  alpha = 0.6){
  my_colors <- adjustcolor(colorRampPalette(cols)(10), alpha.f = alpha)
  ...
  if(long_legend){
    legend_colors <- adjustcolor(colorRampPalette(my_colors)(25), alpha.f = alpha)
    legend(legend_pos, # position
           legend = c(min(round(my_quantiles, digits = legend_round)),
                      rep(NA, 5),
                      round(my_quantiles[3], digits = legend_round),
                      rep(NA, 5),
                      median(round(my_quantiles, digits = legend_round)),
                      rep(NA, 5),
                      round(my_quantiles[7], digits = legend_round),
                      rep(NA, 5),                      
